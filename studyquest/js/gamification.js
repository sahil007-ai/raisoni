...
// Get level info (name, etc.) for a given level number
...
const awardXP = (amount, reason = '') => {
    ...
    // Trigger level-up overlay if level increased
    if (newLevel > oldLevel) {
        showLevelUp(newLevel);
    }

    // Check badges after every XP award
    checkBadges();

    // Update header to reflect new XP
    renderHeader();
...
const checkBadges = () => {
    ...
    BADGE_DEFS.forEach(def => {
        ...
        // If badge not yet earned, check condition
        if (!existing || !existing.earned) {
            if (def.check()) {
                // Badge earned! Update or add
                const badgeData = {
                    id: def.id,
                    name: def.name,
                    icon: def.icon,
                ...
                // Show unlock toast and confetti
