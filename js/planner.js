...
const saveCourseAndGenerate = () => {
    ...
    if (typeof saveCoursesToApi === 'function') saveCoursesToApi(parsedCourses);
    ...
    if (typeof saveSubjectsToApi === 'function') saveSubjectsToApi(subjects);
    ...
    if (typeof saveScheduleToApi === 'function') saveScheduleToApi(generatedSchedule);

    // Award XP for generating schedule
    const count = getData(KEYS.SCHEDULES_GENERATED, 0) + 1;
    saveData(KEYS.SCHEDULES_GENERATED, count);
    syncProgressToBackend({ schedules_generated: count });
...
const markSlotDone = (date, topicId) => {
    ...
    saveData(COURSE_KEY, parsedCourses);
    if (typeof saveScheduleToApi === 'function') saveScheduleToApi(schedule);
    if (typeof saveCoursesToApi === 'function') saveCoursesToApi(parsedCourses);

    // Track study hours
    const totalH = getData(KEYS.TOTAL_STUDY_HOURS, 0);
    const newTotalHours = totalH + ((slot.duration || 60) / 60);
    saveData(KEYS.TOTAL_STUDY_HOURS, newTotalHours);
    syncProgressToBackend({ total_study_hours: newTotalHours });
